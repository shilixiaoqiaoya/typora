...
- **对于单页面应用，访问前端路由时，全部返回index.html，由前端js处理路由渲染出对应界面**
...
- 以流的方式解析 `Content-Type `为 `application/json` 的请求体
- 将字段挂载在req.body.xxx上
...
- 反向代理
  - 接收客户端流量，负载均衡器会自动将流量路由到服务器之一，有很多不同的算法决定路由到哪个服务器
  - **可以避免某台服务器请求压力过大；某一台服务器宕机，还有其他服务器可以处理客户端请求；拦截恶意流量**
  - 本质上也是server
  - 代理服务器可以拦截请求，修改header头信息等等
...
- 视频码率 = 分辨率 * 帧率 * 每像素比特数
  - 分辨率决定画面清晰度（像素数量）。常见的宽高比16:9、4:3。常见的分辨率1080p、2k
  - 帧率决定画面流畅度（每秒帧数）
  - 码率固定时，需要分辨率和帧率间权衡
  - rgb：3个字节；rgba：4个字节
...
屏幕的每个像素，是由红绿蓝的三个发光二极管组成的

<img src="https://cdn.jsdelivr.net/gh/shilixiaoqiaoya/pictures@master/image-20260106182057560.png" alt="image-20260106182057560" style="zoom:25%;" />
...
- yuv格式为yuv420；分辨率是640*480；帧率是15；原始码率计算结果是640x480x1.5x15x8=55,296,000 
- 经过编码器压缩后，数据量大幅减少。h264的压缩比例是100:1，对于上述视频h264建议码率是500kbps



GOP（group of picture）

<img src="https://cdn.jsdelivr.net/gh/shilixiaoqiaoya/pictures@master/image-20260108103718732.png" alt="image-20260108103718732" style="zoom:30%;" /> 

- 每个GOP中，帧与帧之间的差别小

- 每个GOP的第一帧叫IDR帧，当GOP以IDR帧开始时，就形成了一个独立的解码单元，便于视频的随机定位、切换和错误恢复

  - 刷新解码器：IDR帧强制清空参考帧列表
  - 随机访问点：可以从任意IDR帧开始正常播放
  - 错误恢复：网络丢包时，下一个IDR帧可以重新同步
  - 码率控制：IDR帧数据量最大，影响整体码率分布

  **在直播应用中，IDR帧的间隔（GOP长度）直接影响视频的延迟和容错能力**



压缩后帧的分类

- I帧，关键帧，采用帧内压缩技术，IDR帧是一种特殊的I帧
- P帧，向前参考帧。压缩时，只参考前面已经处理的帧，采用帧间压缩技术。它占I帧的一半大小
- B帧，双向参考帧。压缩时，既参考前面已经处理的帧，也参考后面的帧，采用帧间压缩技术。它占I帧的1/4大小
...
任务的主要时间和计算资源都花费在了输入/输出操作的等待上，而不是数据的计算处理上

这里的’等待‘，是指任务需要依赖外部系统的响应，在等待期间，cpu是空闲的或负载极低的，可以处理其它请求

- 数据库操作：代码发送一条SQL查询给数据库，在数据库执行查询并返回结果的几十甚至几百毫秒，node进程只是在等待数据库这个’外部系统‘的响应
- 调用外部API：node服务需要调用另一个服务的restful api(比如获取天气)，在对方服务处理请求并返回数据的这段时间，cpu主要在等待网络传输
- 读写文件：node服务读取磁盘上的一个大文件，在磁盘磁头寻道，旋转盘片，传输数据到内存的这段时间，cpu大部分时间在等待慢速的磁盘I/O

```js
app.get('/io-intensive', async (req, res) => {
  // 这行代码发出请求后，node线程就去处理其他任务了
  // 绝大部分时间在等待api的响应，而不是计算
...
- 复杂的数学计算：计算斐波那契数列、物理模拟、加密解密
- 图像/视频处理：调整图片大小、应用滤镜、视频转码。这需要CPU对每一个像素进行大量运算
- 大数据集处理：在内存中对一个巨大的JSON数组或矩阵进行遍历、排序、聚合

```js
app.get('/cpu-intensive', (req, res) => {
  // 这行代码执行时，cpu需要持续不断地进行递归计算，在此期间，事件循环被阻塞，无法处理其他请求
  const res = fibonacci(40)  // 斐波那契数列
  res.send(`res: ${res}`)
})
```
...
- 编译型语言：直接编译为机器码，执行效率极高
- 强大的多线程支持：轻松利用多核cpu并行计算
- 接近硬件的控制能力：允许精细控制内存和cpu资源
- c++是性能之王，适合对性能有极致追求的场景
  - 游戏引擎、浏览器引擎、高频交易系统
...
java服务：一个餐厅为每一桌新客人雇一个服务员（创建新线程）。客人很多时，需要雇很多服务员（高内存开销），管理起来也复杂

node服务：假设同一时刻有3个请求到达

- 请求A：从数据库查询用户信息（I/O操作）

- 请求B:  读取本地一个大文件（I/O操作）

- 请求C：进行复杂的cpu计算（比如计算斐波那契数列）

对于请求A和请求B，主线程会将任务交给其它

对于请求C，主线程会被同步的cpu计算任务阻塞，在此期间，它无法处理新的请求，无法去执行任务队列中的回调函数



- I/O密集型任务	
  - 数据库查询、API调用、文件读写
  - node单线程可以轻松处理数万并发连接，因为主线程只是快速分发任务，不等待结果
- CPU密集型任务
  - 复杂计算、图像处理
  - 一个计算任务就会阻塞整个事件循环，导致所有其他请求被挂起
...
- 绝对不要阻塞事件循环：避免在主线程执行繁重计算，避免执行同步I/O操作（如 `fs.readFileSync`）
- 利用集群（cluster）
  - 一个node.js实例只能用一个cpu核心
  - 可以使用cluster模块，启动多个子进程，每个进程运行在一个cpu core上，并共享一个端口
  - 一个实例被cpu计算阻塞时，其它实例还可以处理请求
- 将cpu密集型任务卸载到工作线程
  - 使用worker_threads模块，将繁重的计算任务丢给独立的工作线程去处理，处理完成后通知主线程，从而不阻塞事件循环
- 结论
  - 当大量请求是I/O密集型，node的非阻塞特性会表现卓越，单线程就能实现高并发
  - 当大量请求是CPU密集型，node单线程特性会成为瓶颈，必须通过集群或工作线程来化解
